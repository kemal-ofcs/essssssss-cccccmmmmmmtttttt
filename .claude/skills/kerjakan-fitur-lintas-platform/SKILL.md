...
3. **Telusuri alur yang ada dari ujung ke ujung, di KEDUA bahasa.** Temukan: logika server TS dan route handler-nya, fungsi Rust dan `#[tauri::command]`-nya di `commands.rs`, gateway-nya, dan **setiap** layar yang membaca ATAU menulis data yang sama (halaman Web, halaman Mobile, kartu Pengaturan). Pakai `grep` pada nama field, bukan tebakan. Untuk domain baru, domain contoh `master_item` (gateway `item.ts`, halaman `items`) adalah pola yang ditiru.
