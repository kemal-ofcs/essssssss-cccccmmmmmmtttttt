...
use super::{config::MobileState, models::CommandError, storage, sync};
...
pub fn current_device_code(state: &MobileState) -> Result<String, CommandError> {
...
fn stored_license(state: &MobileState) -> Result<Option<String>, CommandError> {
...
async fn refresh_from_cloud(state: &MobileState) -> Result<(), CommandError> {
...
pub async fn resolve(state: &MobileState) -> Result<(Evaluation, String), CommandError> {
...
pub async fn status(state: &MobileState) -> Result<LicenseStatus, CommandError> {
...
pub async fn gate_login(state: &MobileState) -> Result<LicenseGrant, CommandError> {
...
fn store_local_and_enqueue(state: &MobileState, text: &str) -> Result<(), CommandError> {
