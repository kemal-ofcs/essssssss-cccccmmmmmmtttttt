 ...
 * sedangkan `getLastUpdate` tidak. `set` aman di sini karena yang dihitung
 * hanya fungsi yang diimpor dari gateway, bukan setter state React.
 *
 ...
 * lewat. Di template, `recordActivity`, `clearTursoConfig`, `setRolePermissions`,
 * dan `adminDisableTwoFactor` lolos tanpa diperiksa sampai `record`, `clear`,
 * `set`, `admin`, `disable`, dan `enable` ditambahkan.
 */
const AWALAN_MUTASI =
	/^(save|create|update|delete|cancel|queue|submit|insert|edit|generate|import|reset|approve|revoke|assign|toggle|purge|drain|backfill|freeze|remove|add|send|apply|restore|promote|link|unlink|bulk|record|clear|set|admin|disable|enable|simpan|hapus|ubah|buat|batalkan|jadikan|kirim|tambah|aktifkan|nonaktifkan|pulihkan|setujui|tolak|pindah|daftarkan|unggah|terbitkan)[A-Z]/;
