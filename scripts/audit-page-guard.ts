...
function adalahHub(sumber: string): boolean {
	const area = new Set(
		[
			...sumber.matchAll(
				/canAccessArea\(\s*user\s*,\s*["']([a-z_]+)["']\s*\)/g,
			),
		].map((cocok) => cocok[1] as string),
	);
...
const hub: string[] = [];
// Halaman `page.tsx` yang memuat data lewat gateway tetapi TIDAK mengaku
// privat. Mereka di luar cakupan audit ini, jadi dicetak di setiap run: audit
// yang diam soal apa yang tidak ia periksa membusuk menjadi lulus palsu.
const takMengaku: string[] = [];
...
for (const workspace of WORKSPACES) {
	for (const berkasAbsolut of kumpulkanTsx(
		...
		if (!mengakuPrivat(sumber)) {
			if (berkas.endsWith("/page.tsx")) takMengaku.push(berkas);
			continue;
		}
...
if (takMengaku.length > 0) {
	heading("Di luar cakupan — memuat data tanpa mengalihkan tamu ke /login");
	for (const berkas of takMengaku) console.log(`  • ${berkas}`);
}
...
if (pelanggaran.length === 0) {
	...
	console.error(
		`❌ ${pelanggaran.length} halaman privat tanpa penjaga otorisasi.\n` +
			...
			'   `router.replace("/")` (mobile — static export tidak punya rute\n' +
			"   /forbidden; `/` meneruskan ke `landingPath`).\n",
	);
	process.exit(1);
}
