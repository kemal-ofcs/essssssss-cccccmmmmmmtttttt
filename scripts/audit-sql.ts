 ...
 *
 * Di proyek turunan pertama template ini, satu fase lolos seluruh gerbang mutu
 * sambil membawa sembilan rujukan kolom yang tidak pernah ada. Saat audit ini
 * dibawa ke template, ia langsung menemukan query ke tabel yang hanya ada di
 * proyek asal (`log_scan`, `backup_karyawan`). Semua gerbang lain memeriksa
 * BENTUK kode — lint memeriksa gaya, typecheck
...
function literalSql(sumber: string, jenis: "rust" | "ts"): string[] {
	...
	// Template literal DAN string kutip ganda. Membaca backtick saja membuat
	// `client.execute({ sql: "SELECT … FROM log_scan …" })` tidak pernah
	// diperiksa, padahal tabelnya tidak ada dan penghapusan operator di Web
	// selalu gagal karenanya. Literal non-SQL tersaring oleh `AWALAN_QUERY`.
	return [
		...[...sumber.matchAll(/`([\s\S]*?)`/g)].map((cocok) => cocok[1] as string),
		...[...sumber.matchAll(/"((?:[^"\\\n]|\\.)*)"/g)].map((cocok) =>
			(cocok[1] as string).replace(/\\n/g, "\n"),
		),
	];
}
...
// Ambangnya diturunkan dari DDL itu sendiri, bukan angka tetap: jumlah tabel
// berbeda di setiap aplikasi turunan.
const tabelDiDdl = new Set(
	[
		...`${storageSource}\n${tursoSource}`.matchAll(
			// `(?!IF\b)`: komentar yang mengutip "`CREATE TABLE IF NOT EXISTS`"
			// dalam backtick tidak boleh terbaca sebagai tabel bernama `IF`.
			/CREATE\s+TABLE(?:\s+IF\s+NOT\s+EXISTS)?\s+(?!IF\b)(\w+)/gi,
		),
	].map((cocok) => cocok[1] as string),
);
if (jumlahTabel === 0 || jumlahTabel < tabelDiDdl.size) {
	console.error(
		`❌ Skema referensi hanya berisi ${jumlahTabel} dari ${tabelDiDdl.size} tabel yang disebut DDL — DDL gagal terbaca. Audit dihentikan.\n`,
	);
	process.exit(1);
}
...
// Modul yang hanya ada di biner Mobile (tidak punya pasangan di web-desktop)
// tidak pernah terperiksa lewat sasaran di atas. Diturunkan dari isi folder,
// bukan daftar nama, supaya modul Mobile berikutnya ikut tanpa diingat siapa pun.
const modulDesktop = new Set(
	fs.readdirSync(path.join(rootDir, "web-desktop/src-tauri/src/desktop")),
);
for (const berkas of kumpulkanBerkas(
	path.join(rootDir, "mobile/src-tauri/src/mobile"),
	".rs",
)) {
	if (!modulDesktop.has(path.basename(berkas))) {
		sasaran.push({ berkas, jenis: "rust" });
	}
}
